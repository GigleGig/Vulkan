...
void vulkan_deivice_destory(vulkan_context* context) {
    ...
    KINFO("Destorying logical device...");
    if(context->device.logical_device) {
        vkDestroyDevice(context->device.logical_device, context->allocator);
        context->device.logical_device = 0;
    }

    // Pyhsical devices are not destoryed.
...
b8 select_physical_device(vulkan_context* context) {
    ...
    for(u32 i = 0; i < physical_device_count; ++i) {
        ...
        if(result) {
            ...
            KINFO("GPU  Driver version: %d.%d.%d",VK_VERSION_MAJOR(properties.driverVersion), VK_VERSION_MINOR(properties.driverVersion), VK_VERSION_PATCH(properties.driverVersion));

            // Vulkan API version.
            KINFO("Vulkan API version: %d.%d.%d",VK_VERSION_MAJOR(properties.apiVersion), VK_VERSION_MINOR(properties.apiVersion), VK_VERSION_PATCH(properties.apiVersion));
            ...
            // context->device.computer_queue_index = queue_info.computer_family_index;
...
b8 physical_device_meets_requirements(
    ...
    out_queue_info->computer_family_index = -1;
    ...
    // Look at each queue and see what queue is supports.
    KINFO("Graphics | Present | Computer | Transfer | Name");
    ...
    for(u32 i = 0; i < queue_family_count; ++i) {
        ...
        if(queue_families[i].queueFlags & VK_QUEUE_COMPUTE_BIT) {
            out_queue_info->computer_family_index = i;
            ++current_transfer_score;
        }

        // Transfer queue?
        if(queue_families[i].queueFlags & VK_QUEUE_TRANSFER_BIT) {
            // Take the index if it is current lowest. This increase the likelyhood that it is a dedicated transfer queue.
    ...
    if(
        ...
        (!requirements->computer || (requirements->computer && out_queue_info->computer_family_index != -1)) &&
        ...
        KTRACE("Computer Family Index: %i", out_queue_info->computer_family_index);
