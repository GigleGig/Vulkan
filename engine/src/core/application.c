...
b8 application_create(game* game_inst) {
    ...
    if(!renderer_initialize(game_inst->app_config.name, &app_state.platform)) {
        KFATAL("Failed to initialize the renderer. Aborting the application");
        return FALSE;
    }
...
b8 application_run() {
    ...
    while (app_state.is_running) {
        ...
        if(!app_state.is_suspended) {
            ...
            // TODO: refractor packet creation
            ...
            // NOTE: Input update/state copying should always be handled after any input should be recorded.
...
b8 application_on_key(u16 code, void* sender, void* listener_inst, event_context context) {
    if(code == EVENT_CODE_KEY_PRESSED) {
        ...
        if(key_code == KEY_ESCAPE) {
            // NOTE: Technically firing an event to itself, but there maybe other listeners.
            event_context data = {};
